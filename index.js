...
let unfundedGamesNumber = GAMES_JSON.reduce( (total, game) => { return total + (game.pledged < game.goal ? 1 : 0); }, 0);

// create a string that explains the number of unfunded games using the ternary operator
let unfundedGamesString = unfundedGamesNumber === 1 ? "game remains" : "games remain";
const displayStr = `A total of $${totalRaised.toLocaleString("en-US")} has been raised for ${GAMES_JSON.length} games. 
                    Currently, ${unfundedGamesNumber} ${unfundedGamesString} unfunded. We need your help to fund these amazing games!`;
...
function searchGames(searchedGame) {

    const searchTerm = searchedGame.toLowerCase();

    const matchingGames = GAMES_JSON.filter((game) => {
        return game.name.toLowerCase().includes(searchedGame);
    });
...
searchInput.style.fontFamily = "Cabin";
searchInput.style.border = "none";
searchInput.style.padding = "1%";
searchInput.style.margin = "1%";
searchInput.style.borderRadius = "7px";

const buttonContainer = document.getElementById("button-container");
buttonContainer.append(searchInput);
